
# Palettes

ggptt_palettes <- list(
  ptt_new = c("#00713D", "#00627D", "#C36B0D", "#330099", "#FF6600",
          "#00A756", "#1FBADB", "#F3CF95", "#6666CC", "#FFCC00"),
  ptt = c("#009346", "#6495ED", "#EE7600", "#8B3E2F", "#8968CD",
          "#7F7F7F", "#6666CC","#00713D", "#00627D", "#C36B0D", "#330099",
          "#FF6600", "#00A756", "#1FBADB", "#F3CF95","#FFCC00", "#FF0000"),
  vnk = c(rgb(0, 111, 185, maxColorValue = 255),
          rgb(10, 187, 236, maxColorValue = 255),
          rgb(122, 136, 144, maxColorValue = 255),
          "black"),
  aw = c("#0046ad", "#00a5af", "#af1469", "#646e6e", "#ffcd00", "#a0c31e", "#f5f0eb")
    # c("#27509b", "#009ab2", "#e46f26", "#5c2f92", "#69c184", "#f5f0eb")
)

use_data(ggptt_palettes, overwrite = TRUE)
